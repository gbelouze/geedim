    ...
    for name in names[1:]:
        ...
        if start_name == name:
            ...
            landsat_regex = re.compile(
                rf"{start_landsat_match.groups()[0]}\d\d{start_landsat_match.groups()[-1]}"
            )
...
def parse_date(date: Union[datetime, str], var_name=None) -> datetime:
    ...
    if isinstance(date, str):
        try:
            ...
            raise ValueError(
                f'{var_name} should be a datetime instance or a string with format: "%Y-%m-%d"'
            )
    return date
...
class MaskedCollection:
    ...
    def __init__(self, ee_collection: ee.ImageCollection, add_props: List[str] = None):
        ...
        if not isinstance(ee_collection, ee.ImageCollection):
            raise TypeError(
                f"`ee_collection` must be an instance of ee.ImageCollection"
            )
        self._name = None
    ...
    def from_name(cls, name: str, add_props: List[str] = None) -> "MaskedCollection":
        ...
        if (name == "COPERNICUS/S2") or (name == "COPERNICUS/S2_SR"):
            ...
            ee_collection = ee.ImageCollection(
                ee.Join.simple().apply(s2_coll, cloud_prob_coll, filt)
            )
        else:
    ...
    def from_list(
        cls,
        image_list: List[Union[str, MaskedImage, ee.Image]],
        add_props: List[str] = None,
    ) -> "MaskedCollection":
        """
        ...
        for image_obj in image_list:
            if isinstance(image_obj, str):
                im_dict_list.append(
                    dict(ee_image=ee.Image(image_obj), id=image_obj, has_date=True)
                )
            elif isinstance(image_obj, ee.Image):
                ee_info = image_obj.getInfo()
                ee_id = ee_info["id"] if "id" in ee_info else None
                has_date = ("properties" in ee_info) and (
                    "system:time_start" in ee_info["properties"]
                )
                im_dict_list.append(
                    dict(ee_image=ee.Image(image_obj), id=ee_id, has_date=has_date)
                )
            elif isinstance(image_obj, BaseImage):
                im_dict_list.append(
                    dict(
                        ee_image=image_obj.ee_image,
                        id=image_obj.id,
                        has_date=image_obj.date is not None,
                    )
                )
            else:
        ...
        if any(
            [
                (im_dict["id"] is None) or (not im_dict["has_date"])
                for im_dict in im_dict_list
            ]
        ):
            raise InputImageError(
                'Image(s) must have "id" and "system:time_start" properties.'
            )
    ...
    def stats_scale(self) -> Union[float, None]:
        ...
        if not self._stats_scale:
            gsds = [
                float(band_dict["gsd"]) for band_dict in self.stac.band_props.values()
            ]
            max_gsd = max(gsds)
            min_gsd = min(gsds)
            self._stats_scale = (
                min_gsd if (max_gsd > 10 * min_gsd) and (min_gsd > 0) else max_gsd
            )
    ...
    def name(self) -> str:
        ...
        if not self._name:
            ...
            self._name = (
                split_id(ee_info["id"])[0] if ee_info and "id" in ee_info else "None"
            )
        return self._name
        ...
        if not self._schema:
            ...
            if self._add_props:
                for add_prop in self._add_props:
                    ...
                    description = (
                        self.stac.descriptions[add_prop]
                        if self.stac and add_prop in self.stac.descriptions
                        else ""
                    )
                    # remove newlines from description and crop to the first sentence
                    description = " ".join(description.strip().splitlines()).split(
                        ". "
                    )[0]
                    self._schema[add_prop] = dict(
                        abbrev=abbreviate(add_prop), description=description
                    )
        return self._schema
    ...
    def schema_table(self) -> str:
        ...
        for prop_name, prop_dict in self.schema.items():
            ...
            table_list.append(
                dict(
                    abbrev=prop_dict["abbrev"], name=prop_name, description=description
                )
            )
        headers = {key: key.upper() for key in table_list[0].keys()}
        return tabulate.tabulate(
            table_list, headers=headers, floatfmt=".2f", tablefmt="simple"
        )
    ...
    def refl_bands(self) -> Union[List[str], None]:
        ...
        return [
            bname
            for bname, bdict in self.stac.band_props.items()
    ...
    def _get_properties(self, ee_collection: ee.ImageCollection, schema=None) -> Dict:
        ...
        try:
            props_list = ee.List(
                ee_collection.iterate(aggregrate_props, ee.List([]))
            ).getInfo()
    ...
    def _get_properties_table(self, properties: Dict, schema: Dict = None) -> str:
        ...
        for im_id, im_prop_dict in properties.items():
            ...
            for prop_name, key_dict in schema.items():
                if prop_name in im_prop_dict:
                    if (
                        prop_name == "system:time_start"
                    ):  # convert timestamp to date string
                        dt = datetime.utcfromtimestamp(im_prop_dict[prop_name] / 1000)
                        abbrev_dict[key_dict["abbrev"]] = datetime.strftime(
                            dt, "%Y-%m-%d %H:%M"
                        )
                    else:
        ...
        return tabulate.tabulate(
            abbrev_props, headers="keys", floatfmt=".2f", tablefmt=_table_fmt
        )
    ...
    def _prepare_for_composite(
        ...
        **kwargs,
        ...
        resampling = (
            ...
            if resampling
            else BaseImage._default_resampling
        )
        ...
        def prepare_image(ee_image: ee.Image):
            ...
            if date and (method in self._sort_methods):
                date_dist = (
                    ee.Number(ee_image.get("system:time_start"))
        ...
        if method in self._sort_methods:
            ...
            if date:
                logger.warning(
                    "`date` is valid for `mosaic`, `q_mosaic` and `medoid` methods only."
                )
            elif region:
                logger.warning(
                    "`region` is valid for `mosaic`, `q_mosaic` and `medoid` methods only."
                )
    ...
    def search(
        ...
        **kwargs
        ...
        if not start_date and not region:
            raise ValueError(
                "At least one of `start_date` or `region` should be specified"
            )

        if not start_date or not region:
            logger.warning(
                "Specifying `start_date` and `region` will improve the search speed."
            )
        ...
        if start_date:
            ...
            if end_date <= start_date:
                raise ValueError(
                    "`end_date` must be at least a day later than `start_date`"
                )
        ...
        if (
            ...
            if fill_portion:
                ee_collection = ee_collection.filter(
                    ee.Filter.gte("FILL_PORTION", fill_portion)
                )
    ...
    def composite(
        self,
        method: Union[CompositeMethod, str] = None,
        mask: bool = True,
        resampling: Union[ResamplingMethod, str] = None,
        ...
        **kwargs
        ...
        if method is None:
            method = (
                CompositeMethod.mosaic
                if self.image_type == MaskedImage
                else CompositeMethod.q_mosaic
            )
        ...
        dates = [
            datetime.utcfromtimestamp(item["system:time_start"] / 1000)
        ...
        comp_image = comp_image.set(
            "system:index", comp_id
        )  # sets 'properties'->'system:index'

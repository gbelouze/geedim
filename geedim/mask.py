...
class MaskedImage(BaseImage):
    ...
    def _add_aux_bands(self, **kwargs):
        ...
        has_no_aux_bands = ee.Number(
            self.ee_image.bandNames().indexOf("FILL_MASK").lt(0)
        )
        ...
        self.ee_image = ee.Image(
            ee.Algorithms.If(
                overwrite,
                self.ee_image.addBands(aux_image, overwrite=True),
                self.ee_image,
            )
        )
    ...
    def _set_region_stats(self, region: Dict = None, scale: float = None):
        ...
        proj = get_projection(
            self.ee_image, min_scale=False
        )  # get projection of minimum scale band
        ...
        stats_image = ee.Image(
            [
                self.ee_image.select("FILL_MASK").unmask(),
        ...
        fill_portion = (
            ...
            .divide(ee.Number(sums.get("REGION_SUM")))
            .multiply(100)
        )
...
class CloudMaskedImage(MaskedImage):
    ...
    def _cloud_dist(
        self, cloudless_mask: ee.Image = None, max_cloud_dist: float = 5000
    ) -> ee.Image:
        ...
        proj = get_projection(
            self.ee_image, min_scale=False
        )  # use maximum scale projection to save processing time
        ...
        cloud_pix = (
            ee.Number(max_cloud_dist).divide(proj.nominalScale()).round()
        )  # cloud_dist in pixels
        ...
        cloud_dist = cloud_dist.reproject(crs=proj, scale=proj.nominalScale()).rename(
            "CLOUD_DIST"
        )

        # Clip cloud_dist to max_cloud_dist.
        cloud_dist = cloud_dist.where(
            cloud_dist.gt(ee.Image(max_cloud_dist / 10)), max_cloud_dist / 10
        )
    ...
    def _set_region_stats(self, region: Dict = None, scale: float = None):
        ...
        stats_image = ee.Image(
            [
                self.ee_image.select(["FILL_MASK", "CLOUDLESS_MASK"]).unmask(),
                ee.Image(1).rename("REGION_SUM"),
            ]
        )  # yapf: disable
        sums = stats_image.reduceRegion(
        ...
        fill_portion = (
            ee.Number(sums.get("FILL_PORTION"))
            .divide(ee.Number(sums.get("REGION_SUM")))
            .multiply(100)
        )
        cloudless_portion = (
            ee.Number(sums.get("CLOUDLESS_PORTION"))
            .divide(ee.Number(sums.get("FILL_PORTION")))
            .multiply(100)
        )

        # set the encapsulated image properties
        region_stats = ee.Dictionary(
            dict(FILL_PORTION=fill_portion, CLOUDLESS_PORTION=cloudless_portion)
        )
...
class LandsatImage(CloudMaskedImage):
    ...
    * LANDSAT/LC09/C02/T1_L2
    ...
    def _aux_image(
        ...
        return ee.Image(
            [fill_mask, cloud_mask, shadow_mask, cloudless_mask, cloud_dist]
        )
...
class Sentinel2ClImage(CloudMaskedImage):
    ...
    def _aux_image(
        ...
        def get_shadow_mask(ee_im, cloud_mask):
            ...
            shadow_azimuth = ee.Number(-90).add(
                ee.Number(ee_im.get("MEAN_SOLAR_AZIMUTH_ANGLE"))
            )
        ...
        cloud_shadow_mask = cloud_shadow_mask.focal_min(20, units="meters").focal_max(
            buffer, units="meters"
        )
        ...
        cloudless_mask = (
            (cloud_shadow_mask.Not()).And(fill_mask).rename("CLOUDLESS_MASK")
        )
        ...
        cloud_dist = self._cloud_dist(
            cloudless_mask=cloudless_mask, max_cloud_dist=max_cloud_dist
        )

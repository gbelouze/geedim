...
def search(
    ...
    if not obj.region and not start_date:
        raise click.BadOptionUsage(
            "start-date / region",
            "Specify at least --start-time and/or a region with --region/--bbox",
        )
...
def export(obj, image_id, type, folder, bbox, region, like, mask, wait, **kwargs):
    ...
    for im in image_list:
        task = im.export(
            im.name, type=type, folder=folder, wait=False, region=obj.region, **kwargs
        )
...
def composite(obj, image_id, mask, method, resampling, bbox, region, date):
    ...
    if len(obj.image_list) == 0:
        raise click.BadOptionUsage(
            "image_id",
            "Either pass --id, or chain this command with a successful ``search``",
        )

...
@pytest.fixture(scope="session")
def landsat_image_ids(
    l4_image_id, l5_image_id, l7_image_id, l8_image_id, l9_image_id
) -> List[str]:
...
def get_image_std(ee_image: ee.Image, region: Dict, std_scale: float):
    ...
    std_image = test_image.reduceNeighborhood(
        reducer="stdDev", kernel=ee.Kernel.square(2)
    ).rename("TEST")

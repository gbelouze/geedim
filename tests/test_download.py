...
def bounds_polygon(
    left: float, bottom: float, right: float, top: float, crs: str = None
):
...
def _test_export_profile(exp_profile, tgt_profile, transform_shape=False):
    ...
    if transform_shape:
        for key in ["width", "height"]:
            assert exp_profile[key] == tgt_profile[key]
        assert exp_profile["transform"][:6] == pytest.approx(
            tgt_profile["transform"][:6], rel=1e-9
        )
    else:
        assert exp_profile["transform"][:6] == pytest.approx(
            tgt_profile["transform"][:6], rel=0.05
        )

    tgt_bounds = windows.bounds(
        windows.Window(0, 0, tgt_profile["width"], tgt_profile["height"]),
        tgt_profile["transform"],
    )
    exp_bounds = windows.bounds(
        windows.Window(0, 0, exp_profile["width"], exp_profile["height"]),
        exp_profile["transform"],
    )
    if transform_shape:
...
def test_s2_props(s2_sr_base_image: BaseImage):
    ...
    assert s2_sr_base_image.date == datetime.utcfromtimestamp(
        s2_sr_base_image.properties["system:time_start"] / 1000
    )
...
def test_band_props(base_image: str, request: pytest.FixtureRequest):
    ...
    assert [bd["name"] for bd in base_image.band_properties] == [
        bd["id"] for bd in base_image._ee_info["bands"]
    ]
...
def test_has_fixed_projection(
    user_base_image: BaseImage, user_fix_base_image: BaseImage, s2_sr_base_image
):
...
@pytest.mark.parametrize(
    "size, exp_str", [(1024, "1.02 KB"), (234.56e6, "234.56 MB"), (1e9, "1.00 GB")]
)
...
def test_prepare_unbounded(
    base_image: BaseImage, params: Dict, request: pytest.FixtureRequest
):
    """
...
def test_prepare_transform_shape(
    base_image: str, param_image: str, request: pytest.FixtureRequest
):
    ...
    exp_params = dict(
        crs=param_image.crs,
        crs_transform=param_image.transform,
        shape=param_image.shape,
        dtype=param_image.dtype,
    )
...
def test_prepare_region_scale(
    base_image: str, param_image: str, region_25ha: dict, request: pytest.FixtureRequest
):
    ...
    exp_params = dict(
        crs=param_image.crs,
        region=param_image.footprint,
        scale=param_image.scale,
        dtype=param_image.dtype,
    )
    exp_image = base_image._prepare_for_export(**exp_params)
    ...
    _test_export_profile(
        exp_image.profile, tgt_profile, base_image.id == param_image.id
    )
    assert exp_image.scale == param_image.scale
    ...
    tgt_footprint_crs = (
        param_image.footprint["crs"]["properties"]["name"]
        if "crs" in param_image.footprint
        else "EPSG:4326"
    )
...
def test_prepare_nodata(
    user_fix_base_image: BaseImage, region_25ha: Dict, dtype: str, exp_nodata: float
):
    """Test BaseImage._prepare_for_download() sets rasterio profile nodata correctly for different dtypes."""
    exp_image, exp_profile = user_fix_base_image._prepare_for_download(
        region=region_25ha, scale=30, dtype=dtype
    )
...
def test_tiles(image_shape: Tuple, tile_shape: Tuple, image_transform: Affine):
    ...
    for tile in tiles[1:]:
        ...
        if tile.window.row_off == prev_tile.window.row_off:
            assert tile.window.col_off == (
                prev_tile.window.col_off + prev_tile.window.width
            )
            assert tile._transform == pytest.approx(
                (prev_tile._transform * Affine.translation(prev_tile.window.width, 0)),
                rel=0.001,
            )
        else:
            assert tile.window.row_off == (
                prev_tile.window.row_off + prev_tile.window.height
            )
            assert tile._transform == pytest.approx(
                prev_tile._transform
                * Affine.translation(
...
def test_download_transform_shape(
    user_base_image: str, tmp_path: pathlib.Path, request: pytest.FixtureRequest
):
    ...
    download_params = dict(
        crs=tgt_prof["crs"],
        crs_transform=tgt_prof["transform"],
        shape=shape,
        dtype=tgt_prof["dtype"],
    )
...
def test_download_region_scale(
    user_base_image: str, tmp_path: pathlib.Path, request: pytest.FixtureRequest
):
...
def test_overviews(
    user_base_image: BaseImage, region_25ha: Dict, tmp_path: pathlib.Path
):
...
@pytest.mark.parametrize(
    "type", [ExportType.drive, ExportType.asset, ExportType.cloud]
)  # yapf: disable

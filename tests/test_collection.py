...
def test_from_list(image_list: str, request):
    ...
    image_list: List = request.getfixturevalue(image_list)
    image_ids = [
        im_obj if isinstance(im_obj, str) else im_obj.id for im_obj in image_list
    ]
    gd_collection = MaskedCollection.from_list(image_list)
    assert gd_collection.properties is not None
    assert gd_collection.schema is not None
    ...
    assert set(gd_collection.schema.keys()) > set(
        list(gd_collection.properties.values())[0].keys()
    )
...
def test_from_list_ee_image(gedi_image_list: List):
    ...
    image_ids = [
        im_obj if isinstance(im_obj, str) else im_obj.id for im_obj in gedi_image_list
    ]
...
def test_from_list_add_props(
    image_list: str, add_props: List, request: pytest.FixtureRequest
):
    """
    ...
    assert all(
        [gd_collection.schema[add_prop]["abbrev"] is not None for add_prop in add_props]
    )
    assert all(
        [
            add_prop in list(gd_collection.properties.values())[0].keys()
...
def test_search(
    ...
    im_dates = np.array(
        [
            datetime.utcfromtimestamp(im_props["system:time_start"] / 1000)
            for im_props in properties.values()
        ]
    )
    # test FILL_PORTION in expected range
    im_fill_portions = np.array(
        [im_props["FILL_PORTION"] for im_props in properties.values()]
    )
    ...
    if is_csmask:  # is a cloud/shadow masked collection
        ...
        assert np.all(im_cl_portions >= cloudless_portion) and np.all(
            im_cl_portions <= 100
        )
    ...
    assert set(searched_collection.schema.keys()) == set(
...
def test_search_mult_kwargs(region_100ha):
    ...
    filt_collection = gd_collection.search(
        start_date, end_date, region_100ha, prob=80, fill_portion=0
    )
    filt_coll_prob80 = filt_collection.search(
        start_date, end_date, region_100ha, prob=80, fill_portion=0
    )
    filt_coll_prob40 = filt_collection.search(
        start_date, end_date, region_100ha, prob=40, fill_portion=0
    )
...
def test_search_custom_filter(region_25ha):
    ...
    end_date = "2022-02-01"
    gd_collection = MaskedCollection.from_name("LANDSAT/LC09/C02/T1_L2")
    kwarg_coll = gd_collection.search(
        start_date, end_date, region_25ha, cloudless_portion=90
    )
...
def test_search_add_props(region_25ha):
    ...
    gd_collection = MaskedCollection.from_name(
        "LANDSAT/LC09/C02/T1_L2", add_props=add_props
    )
    ...
    assert all(
        [
            add_prop in list(searched_coll.properties.values())[0].keys()
            for add_prop in add_props
        ]
    )
...
def test_composite_region_date_ordering(image_list, method, region, date, request):
    ...
    gd_collection = MaskedCollection.from_list(image_list)
    ee_collection = gd_collection._prepare_for_composite(
        method=method, date=date, region=region
    )
    ...
    if region:
        ...
        portion_key = (
            "CLOUDLESS_PORTION"
            if "CLOUDLESS_PORTION" in schema_keys
            else "FILL_PORTION"
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("s2_sr_image_list", CompositeMethod.medoid, True),
        ("s2_sr_image_list", CompositeMethod.medoid, False),
...
def test_composite_mask(image_list, method, mask, region_100ha, request):
    ...
    component_mask_counts = ee_collection.iterate(
        count_masked_pixels, ee.List([])
    ).getInfo()
    # get the mask pixel count for the composite image
    composite_im = gd_collection.composite(method=method, mask=mask)
    composite_mask_count = (
        count_masked_pixels(composite_im.ee_image, []).get(0).getInfo()
    )
    if mask:
        ...
        assert np.array(component_mask_counts) == pytest.approx(
            composite_mask_count, rel=0.01
        )
...
def test_composite_resampling(
    ...
    comp_im_after = gd_collection.composite(
...
def test_composite_errors(gedi_image_list, region_100ha):
    ...
    with pytest.raises(ValueError):
        ...
        empty_collection = gedi_collection.search(
            "2000-01-01", "2000-01-02", region_100ha, 100
        )

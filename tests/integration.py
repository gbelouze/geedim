...
def test_geeml_integration(tmp_path: Path):
    ...
    with rio.open(out_file, "r") as ds:
        ...
        region_bounds = rio.coords.BoundingBox(
            *region_cnrs.min(axis=0), *region_cnrs.max(axis=0)
        )
...
def test_cli_asset_export(
    l8_image_id, region_25ha_file: Path, runner: CliRunner, tmp_path: Path
):

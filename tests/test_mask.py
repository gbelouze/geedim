...
from geedim.mask import MaskedImage, class_from_id, get_projection


def test_class_from_id(
    landsat_image_ids, s2_sr_image_id, s2_toa_hm_image_id, generic_image_ids
):
    """Test class_from_id()."""
...
def test_set_region_stats(
    masked_image: str, region_100ha, request: pytest.FixtureRequest
):
    """
    ...
    for stat_name in ["FILL_PORTION", "CLOUDLESS_PORTION"]:
        ...
        assert (
            masked_image.properties[stat_name] >= 0
            and masked_image.properties[stat_name] <= 100
        )


@pytest.mark.parametrize(
    "image_id",
    ["l9_image_id", "l8_image_id", "l7_image_id", "l5_image_id", "l4_image_id"],
)
...
@pytest.mark.parametrize(
    "image_id",
    ["s2_toa_image_id", "s2_sr_image_id", "s2_toa_hm_image_id", "s2_sr_hm_image_id"],
)
def test_s2_cloudless_portion(image_id: str, request: pytest.FixtureRequest):
    ...
    masked_image = MaskedImage.from_id(
        image_id, mask_method="qa", mask_shadows=False, mask_cirrus=False
    )
    masked_image._set_region_stats()
    # S2 provided cloudless portion
    s2_cloudless_portion = 100 - float(
        masked_image.properties["CLOUDY_PIXEL_PERCENTAGE"]
    )
...
@pytest.mark.parametrize(
    "image_id, max_cloud_dist", [("s2_sr_image_id", 100), ("s2_sr_hm_image_id", 500)]
)
def test_s2_clouddist_max(
    image_id: str,
    max_cloud_dist: int,
    region_10000ha: Dict,
...
def get_mask_stats(masked_image: MaskedImage, region: Dict):
    ...
    stats_image = ee.Image(
        [pan_cloud, pan_shadow, pan_cloudless, cdist_cloud, cdist_cloudless]
    )
...
def test_s2_aux_bands(
    ...
    assert stats["CDIST_CLOUDLESS"] > stats["CDIST_CLOUD"]
    proj_scale = (
        get_projection(masked_image.ee_image, min_scale=False).nominalScale().getInfo()
    )
...
def test_mask_clouds(
    ...
    masked_image.download(
        filename, region=region_100ha, dtype="int32", scale=proj_scale
    )
    assert filename.exists()
    with rio.open(filename, "r") as ds:
        ds: rio.DatasetReader = ds
        cloudless_mask = ds.read(
            ds.descriptions.index("CLOUDLESS_MASK") + 1, masked=True
        )
        assert np.all(
            cloudless_mask == 1
        )  # all cloud/shadow areas should be masked (0)
        cloudless_mask = cloudless_mask.filled(0).astype(
            "bool"
        )  # fill nodata with 0 and cast to bool

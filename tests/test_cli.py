...
from geedim.utils import asset_id, root_path
...
def _test_downloaded_file(
    ...
    with rio.open(filename, "r") as ds:
        ...
        if region:
            ...
            assert (
                (ds.bounds[0] <= exp_bounds[0])
                and (ds.bounds[1] <= exp_bounds[1])
                and (ds.bounds[2] >= exp_bounds[2])
        ...
        if bands:
            ...
            assert set(bands) == set(
                [ds.tags(bi)["name"] for bi in range(1, ds.count + 1)]
            )
...
def test_config_search_s2(
    ...
    for prob in [40, 80]:
        ...
        cl_portion_list.append(
            np.array(
                [prop_dict["CLOUDLESS_PORTION"] for prop_dict in properties.values()]
            )
        )
...
def test_search_add_props_l9(
    ...
    cli_str = f"search -c {name} -s 2022-01-01 -e 2022-02-01 -r {region_25ha_file} {add_props_str} -op {results_file}"
    result = runner.invoke(cli, cli_str.split())
    assert result.exit_code == 0
    assert results_file.exists()
    with open(results_file, "r") as f:
...
def test_search_cloudless_portion_no_value(
    ...
    for post_fix, cp_spec in zip(["no_val", "zero_val"], ["-cp", "-cp 0"]):
        cli_str = f"search -c {name} -s 2022-01-01 -e 2022-04-01 -r {region_25ha_file} {cp_spec} -op {results_file}"
        result = runner.invoke(cli, cli_str.split())
        assert result.exit_code == 0
        assert results_file.exists()
        with open(results_file, "r") as f:
...
def test_download_crs_transform(
    ...
    _test_downloaded_file(
        out_file, crs=crs, region=region, transform=crs_transform, shape=shape
    )
...
def test_max_tile_size_error(
    ...
    cli_str = (
        f"download -i {s2_sr_image_id} -r {region_100ha_file} -dd {tmp_path} -mts 100"
    )
    result = runner.invoke(cli, cli_str.split())
    assert result.exit_code != 0
...
@pytest.mark.parametrize(
    "image_list, scale", [("s2_sr_image_id_list", 10), ("l8_9_image_id_list", 30)]
)
def test_composite_defaults(
    ...
    cli_str = (
        f"composite -i {image_ids_str} download --crs EPSG:3857 --scale {scale} -r {region_25ha_file} -dd"
        f" {tmp_path}"
    )
    result = runner.invoke(cli, cli_str.split())
    assert result.exit_code == 0
...
def test_composite_params(
    ...
    image_ids_str = " -i ".join(image_list)
    cli_comp_str = (
        f"composite -i {image_ids_str} -cm {method} --resampling {resampling}"
    )
    ...
    with open(region_25ha_file) as f:
        region = json.load(f)
    _test_downloaded_file(
        out_files[0], region=region, crs="EPSG:3857", scale=download_scale
    )


def test_search_composite_download(
    region_25ha_file, runner: CliRunner, tmp_path: pathlib.Path
):
...
def test_search_composite_x2_download(
    region_25ha_file, runner: CliRunner, tmp_path: pathlib.Path
):
    """
    ...
    cli_search_str = (
        f"search -c l7-c2-l2 -s 2022-01-15 -e 2022-04-01 -r {region_25ha_file} -cp 20"
    )
